...
- ADAPT-04/TRANSP-03 genuinely wired (not just backend-tested): a client-initiated weekly adaptation check fires from AppLayout once per 7 days (fire-and-forget, retried on failure), and a readable Adaptations log section on ProgressScreen renders past adaptation decisions — Phase 13 (6/6 must-haves verified; closed a milestone-audit-surfaced gap where both endpoints existed and passed unit tests but had zero real callers; also fixed a pre-existing stale `Adaptation` TS interface and a code-review-caught concurrency bug that could have double-applied an adaptation on a remount or second tab)

### Active

None yet. v1.0 fully shipped; next milestone requirements not yet defined — run `/gsd-new-milestone` to start v1.1 questioning/research/requirements.
...
- **v1.0 shipped state** (2026-07-10): 13 phases, 94 plans, ~220 tasks, ~19,400 LOC (frontend TS/TSX + backend Python), 21-day build. 166 frontend tests, 359 backend tests, all green. Milestone audit (`.planning/v1.0-MILESTONE-AUDIT.md`) confirms 69/69 requirements genuinely reachable by a user (not just unit-tested), all 13 phases verified passed, no blockers.
- **Known tech debt carried into v1.1** (non-blocking, see milestone audit for detail): IOS-03 physical-device wake-lock/safe-area retest never done on real hardware; two Phase 9 manual-only checks (ZWO export gesture timing, AppLayout scroll/pin); `frontend/src/screens/HistoryScreen.tsx` + its test are dead code from the Phase 12 IA redesign, safe to delete; a handful of Phase 12 code-review findings (JWT-in-SSE-URL, a `RideRow` timezone bug, type-safety bypasses, duplicated JSX); 09-VERIFICATION.md has a cosmetic frontmatter/body status mismatch.
...
| Client-triggered lazy check over Vercel Cron for ADAPT-04 | Single-user personal app; a scheduled cron would need a shared-secret auth bypass around `get_current_user` for no material benefit; matches original 03-RESEARCH.md design intent | Validated Phase 13: `useAdaptationCheck` fires from AppLayout (mounts once per session), throttled via a localStorage timestamp; code review caught and fixed a real concurrency gap (duplicate fire on remount/second tab) before merge |
